...
const HINTS: Record<string, string> = {
  ...
  "publish.tiktok.mode":
    "Официальный API: draft — ролик приходит во «Входящие» приложения TikTok, " +
    "опубликовать его нужно одним касанием с телефона (работает сразу); direct — " +
    "сразу в профиль, но пока приложение не прошло аудит TikTok, выложенное так " +
    "видно только автору. Аккаунт через свой браузер: direct жмёт «Опубликовать» " +
    "сразу; draft заливает ролик с подписью и оставляет его локальным черновиком " +
    "TikTok Studio (кнопки «сохранить черновик» в вебе нет) — открой Studio, " +
    "«Продолжить» и опубликуй руками.",
  "publish.tiktok.privacy":
    "Только для режима direct официального API и только из того, что разрешено " +
    "самому аккаунту. Пусто — взять самый закрытый из доступных. Для аккаунта " +
    "через свой браузер не действует: приватность выбирается в TikTok Studio.",
  "publish.tiktok.headless":
    "Только для аккаунтов через свой браузер (Patchright): гонять выкладку " +
    "скрытым окном. Выключи, если хочешь видеть процесс или если TikTok в " +
    "headless просит проверку.",
  "ingest.proxy":
